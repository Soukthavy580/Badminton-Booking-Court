...
// ── REVENUE ──────────────────────────────────────────────────
try {
    // Full price revenue (confirmed bookings)
    $stmt = $pdo->prepare("
        SELECT COALESCE(SUM(
...
try {
    $stmt = $pdo->prepare("
        SELECT
            ...
            MIN(bd.Start_time) AS first_slot,
            MAX(bd.End_time)   AS last_slot,
...
<head>
    ...
    <title>Reports - CourtBook Owner</title>
    ...
    <div class="flex-1 flex flex-col">
        ...
        <header class="bg-white shadow-sm px-6 py-4 sticky top-0 z-40">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-sm text-gray-500"><?= htmlspecialchars($venue['VN_Name']) ?> — Performance Overview</p>
                </div>
                <!-- Period Filter -->
        ...
        <main class="flex-1 p-6">
            ...
            <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
                <?php
                $stat_cards = [
                    ['label'=>'Total Bookings',  'value'=>$stats['total'],     'icon'=>'fa-calendar',       'color'=>'blue',   'sub'=>'in period'],
                    ['label'=>'Confirmed',        'value'=>$stats['confirmed'], 'icon'=>'fa-check-circle',   'color'=>'green',  'sub'=>'bookings'],
                    ['label'=>'Cancelled',        'value'=>$stats['cancelled'], 'icon'=>'fa-times-circle',   'color'=>'red',    'sub'=>'bookings'],
                    ['label'=>'Pending',          'value'=>$stats['pending'],   'icon'=>'fa-clock',          'color'=>'yellow', 'sub'=>'bookings'],
                ];
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
                <div class="stat-card bg-gradient-to-br from-green-500 to-green-600 rounded-2xl shadow-sm p-6 text-white">
                    ...
                    <p class="text-green-100 text-sm mt-1">Total Revenue (Full Price)</p>
                    <p class="text-green-200 text-xs mt-0.5">From confirmed bookings</p>
                </div>
                <div class="stat-card bg-gradient-to-br from-blue-500 to-blue-600 rounded-2xl shadow-sm p-6 text-white">
                    ...
                    <p class="text-blue-100 text-sm mt-1">Deposits Collected (30%)</p>
                    <p class="text-blue-200 text-xs mt-0.5">Paid online by customers</p>
                </div>
                <div class="stat-card bg-gradient-to-br from-purple-500 to-purple-600 rounded-2xl shadow-sm p-6 text-white">
                    ...
                    <p class="text-purple-100 text-sm mt-1">Due at Venue (70%)</p>
                    <p class="text-purple-200 text-xs mt-0.5">Collected in person</p>
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
                ...
                <div class="md:col-span-2 bg-white rounded-2xl shadow-sm p-6">
                    <h2 class="font-bold text-gray-800 mb-4">
                        <i class="fas fa-chart-line text-blue-500 mr-2"></i>Monthly Bookings & Revenue
                    </h2>
                ...
                <div class="bg-white rounded-2xl shadow-sm p-6">
                    <h2 class="font-bold text-gray-800 mb-4">
                        <i class="fas fa-trophy text-yellow-500 mr-2"></i>Court Performance
                    </h2>
                    <?php if (!empty($court_stats)): ?>
                        <div class="space-y-4">
                            <?php foreach ($court_stats as $i => $court):
                                $pct = $max_bookings > 0
                                    ? round(($court['booking_count'] / $max_bookings) * 100)
                                    : 0;
                                $bar_color = match($i) {
                                    0 => 'bg-yellow-400',
                                    1 => 'bg-gray-400',
                                ...
                                <div>
                                    <div class="flex items-center justify-between text-sm mb-1">
                                        <span class="font-semibold text-gray-700">
                                            <?= $medal ?> <?= htmlspecialchars($court['COURT_Name']) ?>
                                        </span>
                                        <span class="text-gray-500 text-xs">
                                            <?= $court['booking_count'] ?> bookings · <?= $court['total_hours'] ?? 0 ?>hrs
                                        </span>
                                    </div>
                    ...
                    <?php else: ?>
                        <div class="text-center py-8">
                            ...
                            <p class="text-gray-400 text-sm">No court data yet</p>
                        </div>
                    <?php endif; ?>
                </div>
            </div>
            ...
            <div class="bg-white rounded-2xl shadow-sm p-6">
                <div class="flex items-center justify-between mb-4">
                    <h2 class="font-bold text-gray-800">
                        <i class="fas fa-list text-gray-500 mr-2"></i>Booking History
                        <span class="text-sm font-normal text-gray-400 ml-2">(last 50)</span>
                    </h2>
                    <span class="text-sm text-gray-400"><?= count($bookings) ?> records</span>
                </div>
                        ...
                        <table class="w-full text-sm">
                            <thead>
                                <tr class="border-b-2 border-gray-100">
                                    ...
                                    <th class="text-left py-3 px-2 text-xs font-bold text-gray-500 uppercase">Customer</th>
                                    <th class="text-left py-3 px-2 text-xs font-bold text-gray-500 uppercase">Courts</th>
                                    <th class="text-left py-3 px-2 text-xs font-bold text-gray-500 uppercase">Date</th>
                                    <th class="text-left py-3 px-2 text-xs font-bold text-gray-500 uppercase">Hours</th>
                                    <th class="text-left py-3 px-2 text-xs font-bold text-gray-500 uppercase">Revenue</th>
                                    <th class="text-left py-3 px-2 text-xs font-bold text-gray-500 uppercase">Status</th>
                                </tr>
                            </thead>
                            <tbody class="divide-y divide-gray-50">
                                <?php foreach ($bookings as $booking):
                                    ...
                                    $booking_revenue = $booking['Status_booking'] === 'Confirmed'
                                        ? $booking['total_hours'] * $price_per_hour
                                        : 0;
                                ?>
                                    <tr class="hover:bg-gray-50 transition">
                                        ...
                                        <td class="py-3 px-2">
                                            <p class="text-gray-700"><?= date('M d, Y', strtotime($booking['first_slot'])) ?></p>
                                            <p class="text-xs text-gray-400">
                                                <?= date('g:i A', strtotime($booking['first_slot'])) ?> -
                                                <?= date('g:i A', strtotime($booking['last_slot'])) ?>
                                            </p>
                                        </td>
                                        <td class="py-3 px-2 text-gray-600"><?= $booking['total_hours'] ?>h</td>
                                        <td class="py-3 px-2">
                                            <?php if ($booking_revenue > 0): ?>
                                                <p class="font-bold text-green-600">₭<?= number_format($booking_revenue) ?></p>
                                                <p class="text-xs text-gray-400">₭<?= number_format($booking_revenue * 0.3) ?> deposit</p>
                                            <?php else: ?>
                ...
                <?php else: ?>
                    <div class="text-center py-10">
                        ...
                        <p class="text-gray-400">No bookings in this period</p>
                    </div>
                <?php endif; ?>
            </div>
...
<script>
    ...
    const bookings = <?= json_encode(array_column($monthly_data, 'confirmed')) ?>;
