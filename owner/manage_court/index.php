        ...
        try {
            ...
            if (!empty($_FILES['vn_image']['name'])) {
                ...
                if (in_array($file['type'], ['image/jpeg', 'image/png', 'image/jpg', 'image/webp']) && $file['size'] <= 5 * 1024 * 1024) {
                    ...
                    $error = 'ຮູບຕ້ອງເປັນ JPG/PNG/WEBP ຂະໜາດບໍ່ເກີນ 5MB.';
                }
            }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['save_court'])) {
    ...
    $tab = 'courts';
    if (!$venue) {
        $error = 'ກະລຸນາບັນທຶກສະຖານທີ່ກ່ອນ.';
    } elseif (!$is_active) {
        $error = 'ທ່ານຕ້ອງການແພັກເກດທີ່ໃຊ້ງານໄດ້ເພື່ອຈັດການເດີ່ນ.';
    } elseif (!$court_name) {
        $error = 'ກະລຸນາໃສ່ຊື່ເດີ່ນ.';
    } else {
        try {
            if ($court_id) {
                $pdo->prepare("UPDATE Court_data SET COURT_Name=? WHERE COURT_ID=? AND VN_ID=?")
                    ->execute([$court_name, $court_id, $venue['VN_ID']]);
                $success = 'ອັບເດດເດີ່ນສຳເລັດ!';
            } else {
                $pdo->prepare("INSERT INTO Court_data (COURT_Name, VN_ID, Court_Status) VALUES (?,?,'Active')")
                    ->execute([$court_name, $venue['VN_ID']]);
...
<body class="bg-gray-50">
                    ...
                    <form method="POST" enctype="multipart/form-data" id="venueForm">
                        <div class="bg-white rounded-2xl shadow-sm p-6 mb-6">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                ...
                                <?php
                                $open_h  = isset($venue['Open_time'])  ? intval(substr($venue['Open_time'],  0, 2)) : 7;
                                $open_m  = isset($venue['Open_time'])  ? intval(substr($venue['Open_time'],  3, 2)) : 0;
                                $close_h = isset($venue['Close_time']) ? intval(substr($venue['Close_time'], 0, 2)) : 22;
                                $close_m = isset($venue['Close_time']) ? intval(substr($venue['Close_time'], 3, 2)) : 0;
                                ?>
                                <div>
                                    <label class="block text-sm font-bold text-gray-700 mb-2">ເວລາເປີດ <span class="text-red-500">*</span></label>
                                    <div class="flex items-center gap-2">
                                        <select id="open_h" class="flex-1 border-2 border-gray-200 rounded-xl px-3 py-3 focus:outline-none focus:border-green-500 transition font-bold text-center">
                                            <?php for ($h = 0; $h < 24; $h++): ?>
                                                <option value="<?= sprintf('%02d', $h) ?>" <?= $open_h === $h ? 'selected' : '' ?>><?= sprintf('%02d', $h) ?></option>
                                            <?php endfor; ?>
                                        </select>
                                        <span class="font-bold text-gray-400 text-lg">:</span>
                                        <select id="open_m" class="flex-1 border-2 border-gray-200 rounded-xl px-3 py-3 focus:outline-none focus:border-green-500 transition font-bold text-center">
                                            <?php foreach (['00', '15', '30', '45'] as $m): ?>
                                                <option value="<?= $m ?>" <?= $open_m === intval($m) ? 'selected' : '' ?>><?= $m ?></option>
                                            <?php endforeach; ?>
                                        </select>
                                    </div>
                                    <input type="hidden" name="open_time" id="open_time_hidden">
                                </div>
                                <div>
                                    <label class="block text-sm font-bold text-gray-700 mb-2">ເວລາປິດ <span class="text-red-500">*</span></label>
                                    <div class="flex items-center gap-2">
                                        <select id="close_h" class="flex-1 border-2 border-gray-200 rounded-xl px-3 py-3 focus:outline-none focus:border-green-500 transition font-bold text-center">
                                            <?php for ($h = 0; $h < 24; $h++): ?>
                                                <option value="<?= sprintf('%02d', $h) ?>" <?= $close_h === $h ? 'selected' : '' ?>><?= sprintf('%02d', $h) ?></option>
                                            <?php endfor; ?>
                                        </select>
                                        <span class="font-bold text-gray-400 text-lg">:</span>
                                        <select id="close_m" class="flex-1 border-2 border-gray-200 rounded-xl px-3 py-3 focus:outline-none focus:border-green-500 transition font-bold text-center">
                                            <?php foreach (['00', '15', '30', '45'] as $m): ?>
                                                <option value="<?= $m ?>" <?= $close_m === intval($m) ? 'selected' : '' ?>><?= $m ?></option>
                                            <?php endforeach; ?>
                                        </select>
                                    </div>
                                    <input type="hidden" name="close_time" id="close_time_hidden">
                                </div>
                                            ...
                                            <div class="flex items-center justify-between gap-4 flex-wrap">
                                                <div class="flex items-center gap-3">
                                                    ...
                                                    <div>
                                                        ...
                                                        <span class="inline-flex items-center gap-1 text-xs font-semibold <?= $cs_cfg['bg'] ?> <?= $cs_cfg['text'] ?> px-2 py-0.5 rounded-full mt-0.5">
                                                ...
                                                <div class="flex items-center gap-2 flex-wrap">
                                                    ...
                                                    <?php endforeach; ?>
                                                    <button onclick="editCourt(<?= $court['COURT_ID'] ?>, '<?= htmlspecialchars(addslashes($court['COURT_Name'])) ?>')"
    ...
    <script>
        ...
        // ── Combine hour+minute selects into hidden inputs before form submits ──
        document.getElementById('venueForm').addEventListener('submit', function () {
            document.getElementById('open_time_hidden').value =
                document.getElementById('open_h').value + ':' +
                document.getElementById('open_m').value + ':00';
            document.getElementById('close_time_hidden').value =
                document.getElementById('close_h').value + ':' +
                document.getElementById('close_m').value + ':00';
        });
    </script>
</body>
</html>