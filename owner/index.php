...
if ($venue) {
    ...
    THEN b.Book_ID END) AS confirmed,
        ...
        // FIX: Revenue includes Confirmed + Completed + No_Show
        // All three = owner received 30% deposit online
        $stmt = $pdo->prepare("
            SELECT SUM(
...
// Recent bookings grouped by Book_ID
$recent_bookings = [];
if ($venue) {
    try {
        $stmt = $pdo->prepare("
        ...
        <div class="flex-1 flex flex-col min-w-0">
            ...
            <main class="flex-1 p-6">
                ...
                <?php else: ?>
                    ...
                    <!-- Stats — revenue now includes Confirmed + Completed + No_Show -->
                        ...
                        <?php foreach (
                            ...
                            <div class="stat-card bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
                                ...
                                <p class="text-2xl font-extrabold text-gray-800">
                                    <?= isset($card['prefix']) ? $card['prefix'] : '' ?><?= $card['fmt'] ? number_format($card['value'], 0) : number_format($card['value']) ?>
                                </p>
                                <p class="text-sm text-gray-500 mt-1"><?= $card['label'] ?></p>
                            </div>
                        <?php endforeach; ?>
                                            ...
                                            <div class="flex items-center justify-between mb-2">
                                                <div class="flex items-center gap-3">
                                                    ...
                                                    <div>
                                                        ...
                                                        <p class="text-xs text-gray-500"><?= $play_date ?> · <?= count($rb['slots']) ?> ເດີ່ນ · #<?= $rb['Book_ID'] ?></p>
                                                    </div>
                                                </div>
