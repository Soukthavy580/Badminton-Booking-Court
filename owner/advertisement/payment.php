...
if (!$rate_id) {
    header('Location: /Badminton_court_Booking/owner/advertisement/index.php');
    exit;
}
...
if (mb_strpos($duration, 'ອາທິດ') !== false) {
    $end_preview = date('d/m/Y', strtotime('+' . ($num * 7) . ' days'));
} elseif (mb_strpos($duration, 'ເດືອນ') !== false) {
    $end_preview = date('d/m/Y', strtotime('+' . $num . ' months'));
} elseif (mb_strpos($duration, 'ປິ') !== false) {
    // FIX: DB stores 'ປິ' (short vowel) not 'ປີ' (long vowel)
    $end_preview = date('d/m/Y', strtotime('+' . $num . ' years'));
} else {
    $end_preview = date('d/m/Y', strtotime('+' . ($num * 7) . ' days'));
}
        ...
        if (move_uploaded_file($file['tmp_name'], $dir . $filename)) {
            try {
                ...
                if (str_contains($duration, 'ອາທິດ')) {
                    $end_dt = date('Y-m-d H:i:s', strtotime('+' . ($num * 7) . ' days'));
                } elseif (str_contains($duration, 'ປີ')) {
                    $end_dt = date('Y-m-d H:i:s', strtotime('+' . $num . ' years'));
                } elseif (str_contains($duration, 'ເດືອນ')) {
                    $end_dt = date('Y-m-d H:i:s', strtotime('+' . $num . ' months'));
                } else {
                    $end_dt = date('Y-m-d H:i:s', strtotime('+' . ($num * 7) . ' days'));
                }
