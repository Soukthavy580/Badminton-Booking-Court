...
if (str_contains($duration, 'Week')) {
    ...
    $end_preview = date('M d, Y', strtotime('+'.intval($m[1]??1).' weeks'));
} elseif (str_contains($duration, 'Year')) {
    $end_preview = date('M d, Y', strtotime('+1 year'));
} else {
    preg_match('/(\d+)/', $duration, $m);
    $end_preview = date('M d, Y', strtotime('+'.intval($m[1]??1).' months'));
}
            ...
            if (move_uploaded_file($file['tmp_name'], $dir . $filename)) {
                try {
                    ...
                    $pdo->prepare("
                        ...
                        VALUES (NOW(), ?, ?, ?, 'Pending', ?, ?)
                    ")->execute([$filename, $start_dt, $end_dt, $vn_id, $rate_id]);

                    // ── Delete ad notification — owner has resubmitted ──
...
<head>
    ...
    <title>Ad Payment - CourtBook</title>
    ...
    <div class="flex-1 flex flex-col">
        <header class="bg-white shadow-sm px-6 py-4 sticky top-0 z-40">
            <div class="flex items-center gap-3">
                ...
                <div>
                    <h1 class="text-xl font-bold text-gray-800">Advertisement Payment</h1>
                    <p class="text-sm text-gray-500">Complete your payment to activate your ad</p>
                </div>
            </div>
        </header>
        ...
        <main class="flex-1 p-6 max-w-2xl mx-auto w-full">
            ...
            <?php if ($success): ?>
                <div class="bg-white rounded-2xl shadow-sm p-10 text-center">
                    ...
                    <p class="text-gray-500 mb-2">Your advertisement payment slip has been submitted successfully.</p>
                    ...
                    <div class="bg-blue-50 border border-blue-200 rounded-xl p-4 mb-6 text-left">
                        <p class="text-sm font-bold text-blue-800 mb-1"><?= htmlspecialchars($rate['Duration']) ?> Advertisement</p>
                        <p class="text-blue-600 text-sm">₭<?= number_format($rate['Price']) ?> · Active until approx. <?= $end_preview ?></p>
                    </div>
                    <a href="/Badminton_court_Booking/owner/advertisement/index.php"
                       class="inline-block bg-blue-600 hover:bg-blue-700 text-white font-bold px-8 py-3 rounded-xl transition">
                        <i class="fas fa-arrow-left mr-2"></i>Back to Advertisements
                    </a>
                </div>
            ...
            <?php else: ?>
                ...
                <div class="bg-gradient-to-r from-blue-600 to-indigo-600 rounded-2xl p-6 mb-6 text-white">
                    <p class="text-blue-200 text-sm mb-1">Selected Plan</p>
                ...
                <div class="bg-white rounded-2xl shadow-sm p-6 mb-6">
                    <h3 class="font-bold text-gray-800 mb-4"><i class="fas fa-qrcode text-blue-500 mr-2"></i>Payment Instructions</h3>
                        ...
                        <div class="flex-1">
                            ...
                            <ol class="space-y-2">
                                <?php foreach ([
                                    ...
                                    'Enter the exact amount: <strong class="text-blue-600">₭'.number_format($rate['Price']).'</strong>',
                ...
                <div class="bg-white rounded-2xl shadow-sm p-6">
                    <h3 class="font-bold text-gray-800 mb-4"><i class="fas fa-upload text-blue-500 mr-2"></i>Upload Payment Screenshot</h3>
                    <form method="POST" enctype="multipart/form-data">
                        ...
                        <button type="submit" class="w-full bg-blue-600 hover:bg-blue-700 text-white font-bold py-4 rounded-xl transition shadow-lg text-lg">
                            <i class="fas fa-paper-plane mr-2"></i>Submit Advertisement Payment
                        </button>
                    </form>
                </div>
