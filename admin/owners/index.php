...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($ca_id && in_array($action, ['ban','unban'])) {
        try {
            ...
            $success = $action === 'ban'
                ? 'Owner account has been banned.'
                : 'Owner account has been reinstated.';
        } catch (PDOException $e) {
            error_log("Owner ban error: " . $e->getMessage());
...
if (isset($_GET['view'])) {
    ...
    try {
        $stmt = $pdo->prepare("
            SELECT co.*, v.VN_ID, v.VN_Name, v.VN_Status,
                   v.VN_Address, v.Price_per_hour,
        ...
        // Package history
        if ($modal_owner && $modal_owner['VN_ID']) {
            $stmt = $pdo->prepare("
            ...
            // Ad history
            $stmt = $pdo->prepare("
                SELECT ad.*, r.Advertisement_duration, r.Price
                FROM advertisement ad
                INNER JOIN rate r ON ad.Rate_ID = r.Rate_ID
                WHERE ad.VN_ID = ?
...
<head>
    ...
    <title>Owners - CourtBook Admin</title>
    ...
    <div class="flex-1 flex flex-col">
        ...
        <header class="bg-white shadow-sm px-6 py-4 sticky top-0 z-40">
            <div class="flex items-center justify-between">
                <div>
                    <h1 class="text-xl font-bold text-gray-800">Owner Management</h1>
                    <p class="text-sm text-gray-500">View and manage court owner accounts</p>
                </div>
                <div class="bg-purple-50 border border-purple-200 px-4 py-2 rounded-xl text-sm">
                    ...
                    <span class="text-purple-500 ml-1">total owners</span>
                </div>
            </div>
        </header>
        ...
        <main class="flex-1 p-6">
            ...
            <div class="grid grid-cols-3 gap-4 mb-6">
                <?php
                $stat_cards = [
                    ['label'=>'All Owners',   'value'=>$counts['all'],    'color'=>'purple', 'icon'=>'fa-user-tie',     'filter'=>'all'],
                    ...
                    ?>
                        <a href="?filter=<?= $key ?><?= $search ? '&search='.urlencode($search) : '' ?>"
                           class="px-4 py-2.5 rounded-xl font-semibold text-sm transition
                            ...
                            <?= $t['label'] ?>
                            <span class="ml-1 text-xs <?= $filter === $key ? 'text-blue-200' : 'text-gray-400' ?>">
                                (<?= $counts[$key] ?>)
                            </span>
                        </a>
                    <?php endforeach; ?>
                        ...
                        <div class="owner-card bg-white rounded-2xl shadow-sm border-l-4 <?= $status_cfg['border'] ?> p-5">
                            <div class="flex flex-col md:flex-row justify-between gap-4">
                                ...
                                <div class="flex items-start gap-4">
                                    ...
                                    <div>
                                        ...
                                        <p class="text-sm text-gray-500">
                                            <i class="fas fa-at mr-1 text-gray-400"></i><?= htmlspecialchars($owner['Username']) ?>
                                        </p>
                                        <p class="text-sm text-gray-500">
                                            <i class="fas fa-envelope mr-1 text-blue-400"></i><?= htmlspecialchars($owner['Email']) ?>
                                        </p>
                                        <p class="text-sm text-gray-500">
                                            <i class="fas fa-phone mr-1 text-green-400"></i><?= htmlspecialchars($owner['Phone']) ?>
                                        </p>
                                    </div>
                                </div>
                                    ...
                                    <?php if ($owner['VN_Name']): ?>
                                        <div class="bg-gray-50 rounded-xl px-4 py-2">
                                            <p class="font-semibold text-gray-700">
                                                ...
                                                <?= htmlspecialchars($owner['VN_Name']) ?>
                                            </p>
                                            <p class="text-gray-400 text-xs mt-0.5">
                                                <i class="fas fa-map-marker-alt mr-1"></i><?= htmlspecialchars($owner['VN_Address']) ?>
                                            </p>
                                            <div class="flex gap-3 mt-1">
                                                ...
                                                ?>
                                                <span class="bg-<?= $vs_color ?>-100 text-<?= $vs_color ?>-700 text-xs px-2 py-0.5 rounded-full font-semibold">
                                                    <?= $vs ?>
                                                </span>
                                                <?php if ($owner['has_active_package']): ?>
                                                    <span class="bg-purple-100 text-purple-700 text-xs px-2 py-0.5 rounded-full font-semibold">
                                                        <i class="fas fa-box mr-1"></i>Package Active
                                                    </span>
                                                <?php endif; ?>
                                                <?php if ($owner['has_active_ad']): ?>
                                        ...
                                        <p class="text-xs text-gray-400 text-right">
                                            ...
                                            <?= $owner['total_bookings'] ?> confirmed bookings
                                        </p>
                                    <?php else: ?>
                                        <div class="bg-gray-50 rounded-xl px-4 py-2 text-gray-400 text-xs">
                                            <i class="fas fa-store mr-1"></i>No venue set up
                                        </div>
                                    <?php endif; ?>
                                </div>
                                ...
                                <div class="flex flex-col gap-2 flex-shrink-0 justify-start">
                                    ...
                                    <?php if ($is_banned): ?>
                                        <form method="POST">
                                            <input type="hidden" name="ca_id" value="<?= $owner['CA_ID'] ?>">
                                            <input type="hidden" name="action" value="unban">
                                            <button type="submit"
                                                    class="w-full bg-green-50 hover:bg-green-100 text-green-600 px-4 py-2 rounded-xl text-sm font-bold transition border border-green-200">
                                                <i class="fas fa-check-circle mr-1"></i>Unban
                                            </button>
                                        </form>
                                    <?php else: ?>
                                        <form method="POST"
                                              onsubmit="return confirm('Ban <?= htmlspecialchars(addslashes($owner['Name'])) ?>? They will not be able to login.')">
                                            <input type="hidden" name="ca_id" value="<?= $owner['CA_ID'] ?>">
                                            <input type="hidden" name="action" value="ban">
                                            <button type="submit"
                                                    class="w-full bg-red-50 hover:bg-red-100 text-red-600 px-4 py-2 rounded-xl text-sm font-bold transition border border-red-200">
            ...
            <?php else: ?>
                <div class="bg-white rounded-2xl shadow-sm p-12 text-center">
                    ...
                    <h3 class="text-xl font-bold text-gray-600 mb-2">No Owners Found</h3>
        ...
        <div class="bg-gradient-to-r from-purple-500 to-blue-600 p-6 rounded-t-2xl relative">
            ...
            <div class="flex items-center gap-4">
                ...
                <div class="text-white">
                    <h2 class="text-2xl font-extrabold"><?= htmlspecialchars($modal_owner['Name'] . ' ' . $modal_owner['Surname']) ?></h2>
                    ...
                    <span class="inline-block mt-1 bg-white bg-opacity-20 text-white text-xs font-bold px-3 py-0.5 rounded-full">
                        <?= htmlspecialchars($modal_owner['Status'] ?? 'Active') ?>
                    </span>
                </div>
            </div>
        </div>
        ...
        <div class="p-6">
            ...
            <?php if ($modal_owner['VN_Name']): ?>
                <div class="bg-blue-50 border border-blue-200 rounded-2xl p-4 mb-6">
                    <h3 class="font-bold text-blue-800 mb-3">
                        <i class="fas fa-store mr-2"></i>Venue Information
                    </h3>
                    <div class="grid grid-cols-2 gap-2 text-sm">
                        <?php
                        $vfields = [
                            ['label'=>'Venue Name',  'value'=>$modal_owner['VN_Name']],
                            ['label'=>'Status',      'value'=>$modal_owner['VN_Status']],
                            ['label'=>'Address',     'value'=>$modal_owner['VN_Address']],
                            ['label'=>'Hours',       'value'=>$modal_owner['Open_time'].' - '.$modal_owner['Close_time']],
                            ['label'=>'Price/hr',    'value'=>'₭'.number_format(preg_replace('/[^0-9]/','',$modal_owner['Price_per_hour']))],
                            ['label'=>'Courts',      'value'=>$modal_owner['total_courts'].' courts'],
                            ['label'=>'Bookings',    'value'=>$modal_owner['total_bookings'].' confirmed'],
                        ];
            ...
            <div class="mb-6">
                <h3 class="font-bold text-gray-800 mb-3">
                    <i class="fas fa-box text-purple-500 mr-2"></i>Package History
                </h3>
                <?php if (!empty($modal_packages)): ?>
                    <div class="space-y-2">
                        <?php foreach ($modal_packages as $pkg):
                            $pkg_color = match($pkg['Status_Package']) {
                                'Active'   => 'green',
                                'Pending'  => 'yellow',
                                'Rejected' => 'red',
                                default    => 'gray'
                        ...
                        ?>
                            <div class="flex items-center justify-between bg-gray-50 rounded-xl px-4 py-3 text-sm">
                                <div class="flex items-center gap-3">
                                    <div class="bg-<?= $pkg_color ?>-100 p-1.5 rounded-lg">
                                        <i class="fas fa-box text-<?= $pkg_color ?>-500 text-xs"></i>
                                    </div>
                                    <div>
                                ...
                                <div class="text-right">
                                    ...
                                    <span class="bg-<?= $pkg_color ?>-100 text-<?= $pkg_color ?>-700 text-xs font-bold px-2 py-0.5 rounded-full">
                                        <?= $pkg['Status_Package'] ?>
                                    </span>
                                </div>
                            </div>
                ...
                <?php else: ?>
                    <div class="text-center py-4 bg-gray-50 rounded-xl">
                        <p class="text-gray-400 text-sm">No package history</p>
                    </div>
                <?php endif; ?>
            </div>
            ...
            <div class="mb-6">
                <h3 class="font-bold text-gray-800 mb-3">
                    <i class="fas fa-bullhorn text-blue-500 mr-2"></i>Advertisement History
                </h3>
                <?php if (!empty($modal_ads)): ?>
                    <div class="space-y-2">
                        <?php foreach ($modal_ads as $ad):
                            $ad_color = match($ad['Status_AD']) {
                        ...
                        ?>
                            <div class="flex items-center justify-between bg-gray-50 rounded-xl px-4 py-3 text-sm">
                                <div class="flex items-center gap-3">
                                    <div class="bg-<?= $ad_color ?>-100 p-1.5 rounded-lg">
                                        <i class="fas fa-bullhorn text-<?= $ad_color ?>-500 text-xs"></i>
                                    </div>
                                    <div>
                                        <p class="font-semibold text-gray-800"><?= htmlspecialchars($ad['Advertisement_duration']) ?></p>
                                        <p class="text-gray-400 text-xs"><?= date('M d, Y', strtotime($ad['AD_date'])) ?>
                                            <?php if ($ad['End_time']): ?>
                                                · until <?= date('M d, Y', strtotime($ad['End_time'])) ?>
                                            <?php endif; ?>
                                        </p>
                                    </div>
                                </div>
                                <div class="text-right">
                                    ...
                                    <span class="bg-<?= $ad_color ?>-100 text-<?= $ad_color ?>-700 text-xs font-bold px-2 py-0.5 rounded-full">
                                        <?= $ad['Status_AD'] ?>
                                    </span>
                                </div>
                            </div>
                ...
                <?php else: ?>
                    <div class="text-center py-4 bg-gray-50 rounded-xl">
                        <p class="text-gray-400 text-sm">No advertisement history</p>
                    </div>
                <?php endif; ?>
            </div>
            ...
            <div class="pt-4 border-t border-gray-100">
                <?php if (($modal_owner['Status'] ?? 'Active') === 'Banned'): ?>
                    <form method="POST">
                        <input type="hidden" name="ca_id" value="<?= $modal_owner['CA_ID'] ?>">
                        <input type="hidden" name="action" value="unban">
                        <button type="submit"
                                class="w-full bg-green-600 hover:bg-green-700 text-white font-bold py-3 rounded-xl transition">
                            <i class="fas fa-check-circle mr-2"></i>Unban This Owner
                        </button>
                    </form>
                <?php else: ?>
                    <form method="POST"
                          onsubmit="return confirm('Ban <?= htmlspecialchars(addslashes($modal_owner['Name'])) ?>? They will not be able to login.')">
                        <input type="hidden" name="ca_id" value="<?= $modal_owner['CA_ID'] ?>">
                        <input type="hidden" name="action" value="ban">
                        <button type="submit"
                                class="w-full bg-red-50 hover:bg-red-100 text-red-600 font-bold py-3 rounded-xl border border-red-200 transition">
                            <i class="fas fa-ban mr-2"></i>Ban This Owner
                        </button>
                    </form>
                <?php endif; ?>
            </div>
