...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($c_id && in_array($action, ['ban','unban'])) {
        try {
            ...
            $success = $action === 'ban'
                ? 'Customer account has been banned.'
                : 'Customer account has been reinstated.';
        } catch (PDOException $e) {
            error_log("Customer ban error: " . $e->getMessage());
    ...
    try {
        ...
        $sql = "
            SELECT
                ...
                SUM(CASE WHEN b.Status_booking = 'Pending'   THEN 1 ELSE 0 END) AS pending_bookings
            FROM customer cu
        ...
        if ($modal_customer) {
            ...
            $stmt = $pdo->prepare("
                SELECT
                    ...
                    c.COURT_Name, v.VN_Name, v.Price_per_hour
                FROM booking b
            ...
            ");
            $stmt->execute([$view_id]);
            $all_booking_rows = $stmt->fetchAll();

            // Group by Book_ID
            $grouped = [];
            foreach ($all_booking_rows as $row) {
                $id = $row['Book_ID'];
                if (!isset($grouped[$id])) {
                    $grouped[$id] = $row;
                    $grouped[$id]['slots'] = [];
...
$counts    = [
    'all'    => count_customers($pdo, 'all'),
    'active' => count_customers($pdo, 'active'),
    'banned' => count_customers($pdo, 'banned'),
];
?>
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <title>Customers - CourtBook Admin</title>
                        ...
                        <div class="customer-card bg-white rounded-2xl shadow-sm border-l-4 <?= $status_cfg['border'] ?> p-5">
                            <div class="flex flex-col md:flex-row justify-between gap-4">
                                ...
                                <div class="flex items-start gap-4">
                                    ...
                                    <div>
                                        ...
                                        <p class="text-sm text-gray-500">
                                            <i class="fas fa-at mr-1 text-gray-400"></i><?= htmlspecialchars($customer['Username']) ?>
                                        </p>
                                        <p class="text-sm text-gray-500">
                                            <i class="fas fa-envelope mr-1 text-blue-400"></i><?= htmlspecialchars($customer['Email']) ?>
                                        </p>
                                        <p class="text-sm text-gray-500">
                                            <i class="fas fa-phone mr-1 text-green-400"></i><?= htmlspecialchars($customer['Phone']) ?>
                                        </p>
                                        <?php if ($customer['Gender']): ?>
                                            <p class="text-sm text-gray-500">
                                                <i class="fas fa-venus-mars mr-1 text-purple-400"></i><?= htmlspecialchars($customer['Gender']) ?>
                                            </p>
                                        <?php endif; ?>
                                    </div>
                                </div>
                                ...
                                <div class="flex flex-col gap-2 flex-shrink-0 justify-start">
                                    ...
                                    <?php else: ?>
                                        <form method="POST"
                                              onsubmit="return confirm('Ban <?= htmlspecialchars(addslashes($customer['Name'])) ?>? They will not be able to login.')">
                                            <input type="hidden" name="c_id" value="<?= $customer['C_ID'] ?>">
        ...
        <div class="bg-gradient-to-r from-yellow-400 to-orange-500 p-6 rounded-t-2xl relative">
            ...
            <div class="flex items-center gap-4">
                ...
                <div class="text-white">
                    <h2 class="text-2xl font-extrabold"><?= htmlspecialchars($modal_customer['Name'] . ' ' . $modal_customer['Surname']) ?></h2>
        ...
        <div class="p-6">
            ...
            <div class="mb-6">
                <h3 class="font-bold text-gray-800 mb-3">
                    <i class="fas fa-calendar-check text-blue-500 mr-2"></i>Booking History
                </h3>
                            ...
                            <div class="bg-gray-50 rounded-xl px-4 py-3 text-sm">
                                <div class="flex items-center justify-between mb-1">
                                    <div class="flex items-center gap-2">
                                        <span class="bg-<?= $b_color ?>-100 text-<?= $b_color ?>-700 text-xs font-bold px-2 py-0.5 rounded-full">
                                ...
                                <?php foreach ($booking['slots'] as $slot): ?>
                                    <p class="text-xs text-gray-500 ml-1">
                                        ...
                                        <?= date('M d, Y', strtotime($slot['start'])) ?> ·
                                        <?= date('g:i A', strtotime($slot['start'])) ?> -
                                        <?= date('g:i A', strtotime($slot['end'])) ?>
                                    </p>
                                <?php endforeach; ?>
                                <p class="text-xs text-gray-400 mt-1">
                                    Booked: <?= date('M d, Y g:i A', strtotime($booking['Booking_date'])) ?>
                                </p>
                            </div>
                        <?php endforeach; ?>
                    </div>
                <?php else: ?>
                    <div class="text-center py-4 bg-gray-50 rounded-xl">
                        <p class="text-gray-400 text-sm">No booking history</p>
                    </div>
                <?php endif; ?>
            </div>
            ...
            <div class="mb-6">
                <h3 class="font-bold text-gray-800 mb-3">
                    <i class="fas fa-times-circle text-red-500 mr-2"></i>Cancellation History
                </h3>
                        ...
                        <?php foreach ($modal_cancellations as $cancel): ?>
                            <div class="bg-red-50 border border-red-100 rounded-xl px-4 py-3 text-sm">
                                ...
                                <p class="text-xs text-gray-500">
                                    ...
                                    <?= date('M d, Y', strtotime($cancel['Start_time'])) ?>
                                </p>
                                <?php if ($cancel['Comment']): ?>
                                    <p class="text-xs text-red-600 mt-1 bg-red-100 rounded-lg px-2 py-1">
                                        <i class="fas fa-comment mr-1"></i>
                                        "<?= htmlspecialchars($cancel['Comment']) ?>"
                                    </p>
                                <?php endif; ?>
                            </div>
                        <?php endforeach; ?>
                ...
                <?php else: ?>
                    <div class="text-center py-4 bg-gray-50 rounded-xl">
                        <p class="text-gray-400 text-sm">No cancellations</p>
                    </div>
                <?php endif; ?>
            </div>
            ...
            <div class="pt-4 border-t border-gray-100">
                <?php if (($modal_customer['Status'] ?? 'Active') === 'Banned'): ?>
                    <form method="POST">
                        <input type="hidden" name="c_id" value="<?= $modal_customer['C_ID'] ?>">
                        <input type="hidden" name="action" value="unban">
                        <button type="submit"
                ...
                <?php else: ?>
                    <form method="POST"
                          onsubmit="return confirm('Ban <?= htmlspecialchars(addslashes($modal_customer['Name'])) ?>? They will not be able to login.')">
                        <input type="hidden" name="c_id" value="<?= $modal_customer['C_ID'] ?>">
                        <input type="hidden" name="action" value="ban">
                        <button type="submit"
