...
function count_packages($pdo, $filter) {
    try {
        $where = match($filter) { 'pending'=>"Status_Package='Pending'",'active'=>"Status_Package='Active' AND End_time>NOW()",'expired'=>"Status_Package='Expired' OR (Status_Package='Active' AND End_time<NOW())",'rejected'=>"Status_Package='Rejected'",default=>'1=1' };
        return (int)$pdo->query("SELECT COUNT(*) FROM package WHERE $where")->fetchColumn();
    } catch (PDOException $e) { return 0; }
}

try { $packages = get_packages($pdo, $filter); } catch (PDOException $e) { $packages = []; $error = 'Query error: '.$e->getMessage(); }
...
try { $pkg_rates = $pdo->query("SELECT * FROM package_rate ORDER BY Price ASC")->fetchAll(); } catch (PDOException $e) { $pkg_rates = []; }
try { $revenue = $pdo->query("SELECT COALESCE(SUM(pr.Price),0) FROM package bp INNER JOIN package_rate pr ON bp.Package_rate_ID=pr.Package_rate_ID WHERE bp.Status_Package='Active'")->fetchColumn(); } catch (PDOException $e) { $revenue = 0; }
?>
<!DOCTYPE html>
<html lang="lo">
<head>
                                ...
                                <div class="flex flex-col items-end gap-1 text-right flex-shrink-0">
                                    ...
                                    <?php if ($pkg['Status_Package']==='Active' && $pkg['End_time']): ?>
                                        ...
                                        <?php else: ?>
                                            ...
                                            <?php $total_days = max(1,(strtotime($pkg['End_time'])-strtotime($pkg['Start_time']))/86400); $pct = min(100,max(0,($days_left/$total_days)*100)); ?>
            ...
            <?php else: ?>
                <div class="bg-white rounded-2xl shadow-sm p-12 text-center">
                    ...
                    <h3 class="text-xl font-bold text-gray-600 mb-2">
                        <?= match($filter) { 'pending'=>'ບໍ່ມີລາຍການລໍຖ້າ','active'=>'ບໍ່ມີແພັກເກດທີ່ໃຊ້ງານໄດ້','expired'=>'ບໍ່ມີແພັກເກດໝົດອາຍຸ',default=>'ບໍ່ມີລາຍການ' } ?>
                    </h3>
...
function openReject(id){document.getElementById('rejectPkgId').value=id;document.querySelector('#rejectModal textarea').value='';document.getElementById('rejectModal').classList.add('open');}
