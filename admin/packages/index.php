...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['save_rate'])) {
    ...
    if ($rate_id && $duration && $price > 0) {
        try {
            ...
            $success = 'Package rate updated successfully.';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['add_rate'])) {
    ...
    if ($duration && $price > 0) {
        try {
            ...
            $success = 'New package rate added.';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['delete_rate'])) {
    ...
    if ($rate_id) {
        try {
            ...
            if ($in_use > 0) {
                $error = 'Cannot delete a rate that is currently in use by active or pending packages.';
            } else {
                $pdo->prepare("DELETE FROM package_rate WHERE Package_rate_ID=?")->execute([$rate_id]);
                $success = 'Package rate deleted.';
            }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['delete_rejected'])) {
    ...
    if ($pkg_id) {
        try {
            ...
            $success = 'Rejected package deleted.';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && !isset($_POST['save_rate']) && !isset($_POST['add_rate']) && !isset($_POST['delete_rate']) && !isset($_POST['delete_rejected'])) {
    ...
    if ($pkg_id && in_array($action, ['approve', 'reject'])) {
        try {
            $stmt = $pdo->prepare("
                SELECT bp.*, pr.Package_duration, co.CA_ID
                FROM package bp
            ...
            if ($pkg) {
                if ($action === 'approve') {
                    ...
                    // ── STACKING: find the latest End_time for this owner's active package ──
                    $latestEnd = null;
                    try {
                        $es = $pdo->prepare("
                            SELECT MAX(End_time) FROM package
                            WHERE CA_ID = ? AND Package_ID != ? AND Status_Package = 'Active' AND End_time > NOW()
                        ");
                        $es->execute([$pkg['CA_ID'], $pkg_id]);
                        $latestEnd = $es->fetchColumn();
                    } catch (PDOException $e) {}

                    // Start from today OR from the end of the latest active package (whichever is later)
                    ...
                    $pdo->prepare("DELETE FROM owner_notification WHERE CA_ID=? AND type='package' AND ref_id=?")
                        ->execute([$pkg['CA_ID'], $pkg_id]);

                    $queued = $start > date('Y-m-d H:i:s');
                    $success = $queued
                        ? 'Package approved and queued! Will activate after current package expires (' . date('M d, Y', strtotime($start)) . ').'
                        : 'Package approved! Owner account is now active until ' . date('M d, Y', strtotime($end)) . '.';

                } else {
                    if (empty($comment)) {
                        $error = 'Please provide a rejection reason for the owner.';
                    } else {
                        $pdo->prepare("UPDATE package SET Status_Package='Rejected' WHERE Package_ID=?")->execute([$pkg_id]);
                        $pdo->prepare("DELETE FROM owner_notification WHERE CA_ID=? AND type='package' AND ref_id=?")->execute([$pkg['CA_ID'], $pkg_id]);
                        $pdo->prepare("INSERT INTO owner_notification (CA_ID, type, ref_id, title, message) VALUES (?, 'package', ?, 'Package Payment Rejected', ?)")
                            ->execute([$pkg['CA_ID'], $pkg_id, $comment]);
                        $success = 'Package rejected. Owner has been notified.';
                    }
                }
            }
        } catch (PDOException $e) {
...
function get_packages($pdo, $filter) {
    ...
    $stmt = $pdo->query("
        SELECT
            bp.*,
            pr.Package_rate_ID, pr.Package_duration, pr.Price,
            COALESCE(pr.Is_Popular, 0)    AS rate_popular,
            COALESCE(pr.Is_Best_Value, 0) AS rate_best,
        ...
        INNER JOIN package_rate pr ON bp.Package_rate_ID = pr.Package_rate_ID
        INNER JOIN court_owner co  ON bp.CA_ID = co.CA_ID
        LEFT JOIN  owner_notification n
               ON n.CA_ID = bp.CA_ID AND n.type = 'package' AND n.ref_id = bp.Package_ID
        WHERE 1=1 $where
...
$counts   = [
    'pending'  => count_packages($pdo, 'pending'),
    'active'   => count_packages($pdo, 'active'),
    'expired'  => count_packages($pdo, 'expired'),
    'rejected' => count_packages($pdo, 'rejected'),
];

try {
    $pkg_rates = $pdo->query("SELECT * FROM package_rate ORDER BY Price ASC")->fetchAll();
} catch (PDOException $e) { $pkg_rates = []; }
...
try {
    $revenue = $pdo->query("
        ...
        FROM package bp
        INNER JOIN package_rate pr ON bp.Package_rate_ID = pr.Package_rate_ID
        WHERE bp.Status_Package = 'Active'
    ")->fetchColumn();
...
<head>
    ...
    <title>Packages - CourtBook Admin</title>
    <script src="https://cdn.tailwindcss.com"></script>
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.4.0/css/all.min.css">
    <style>
        .pkg-card { transition: all 0.3s ease; }
        .pkg-card:hover { transform: translateY(-2px); box-shadow: 0 8px 20px rgba(0,0,0,0.08); }
    ...
    <div class="flex-1 flex flex-col">
        <header class="bg-white shadow-sm px-6 py-4 sticky top-0 z-40">
            <div class="flex items-center justify-between">
                <div>
                    <h1 class="text-xl font-bold text-gray-800">Package Management</h1>
                    <p class="text-sm text-gray-500">Review, approve, and manage package rates</p>
                </div>
                <div class="flex items-center gap-3">
                    <div class="bg-green-50 border border-green-200 px-4 py-2 rounded-xl text-sm">
                        ...
                        <span class="text-green-500 ml-1">total revenue</span>
                    </div>
        ...
        <main class="flex-1 p-6">
            ...
            <!-- ══ PACKAGE RATES MANAGEMENT PANEL ══ -->
            <div class="bg-white rounded-2xl shadow-sm mb-6 overflow-hidden border border-gray-100">
                <button onclick="toggleRates()" class="w-full flex items-center justify-between px-6 py-4 hover:bg-gray-50 transition">
                    <div class="flex items-center gap-3">
                        <div class="bg-green-100 w-9 h-9 rounded-xl flex items-center justify-center">
                            <i class="fas fa-box text-green-500"></i>
                        </div>
                        <div class="text-left">
                            <h2 class="font-bold text-gray-800">Package Rate Plans</h2>
                            <p class="text-xs text-gray-400"><?= count($pkg_rates) ?> plan<?= count($pkg_rates) != 1 ? 's' : '' ?> · Click to manage prices, durations and descriptions</p>
                        </div>
                    </div>
                    ...
                    <div class="p-6">
                        ...
                        <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4 mb-2">
                            <?php foreach ($pkg_rates as $rate):
                                ...
                                $mo_price = preg_match('/(\d+)/', $rate['Package_duration'], $m)
                                    ? round($rate['Price'] / max(1, intval($m[1]))) : null;
                            ?>
                            <div class="relative bg-white rounded-2xl border-2 <?= $is_pop ? 'border-blue-500 shadow-lg' : ($is_best ? 'border-yellow-400 shadow-lg' : 'border-gray-200') ?> p-4 flex flex-col items-center text-center pt-6">
                                ...
                                <form method="POST" class="w-full space-y-1">
                                    ...
                                    <input type="text" name="rate_duration"
                                           value="<?= htmlspecialchars($rate['Package_duration']) ?>"
                                    ...
                                    <?php if ($mo_price): ?>
                                        <p class="text-xs text-gray-400">₭<?= number_format($mo_price) ?>/mo</p>
                                    <?php endif; ?>
                                    ...
                                    <div class="flex gap-1.5 pt-1">
                                        <button type="button"
                                                onclick="toggleBadge('pop_pkg_<?= $rate['Package_rate_ID'] ?>', 'best_pkg_<?= $rate['Package_rate_ID'] ?>', 'popular', this)"
                                        ...
                                        <button type="button"
                                                onclick="toggleBadge('best_pkg_<?= $rate['Package_rate_ID'] ?>', 'pop_pkg_<?= $rate['Package_rate_ID'] ?>', 'best', this)"
                            ...
                            <div class="bg-white rounded-2xl border-2 border-dashed border-green-200 p-4 flex flex-col items-center justify-center text-center gap-2 min-h-[260px]">
                                ...
                                <p class="text-xs font-bold text-green-600">New Plan</p>
                                <form method="POST" class="w-full space-y-1.5">
                                    <input type="hidden" name="is_popular" value="0">
                                    <input type="hidden" name="is_best_value" value="0">
                                    <input type="text" name="rate_duration" placeholder="Duration (e.g. 3 Months)"
                                           class="w-full text-center text-xs border border-gray-200 rounded-lg px-2 py-1.5 focus:outline-none focus:border-green-400">
                                    <input type="text" name="rate_price" placeholder="Price (₭)"
                                           class="w-full text-center text-xs border border-gray-200 rounded-lg px-2 py-1.5 focus:outline-none focus:border-green-400">
                                    <button type="submit" name="add_rate"
                                            class="w-full bg-green-600 hover:bg-green-700 text-white font-bold py-2 rounded-xl text-xs transition mt-1">
                        ...
                        <div class="bg-white rounded-2xl border-2 border-dashed border-green-200 p-6 w-64 text-center space-y-2">
                            ...
                            <p class="text-sm font-bold text-green-600">Add First Plan</p>
                            <form method="POST" class="space-y-1.5">
                                ...
                                <input type="text" name="rate_price" placeholder="Price (₭)"
                                       class="w-full text-center text-xs border border-gray-200 rounded-lg px-2 py-1.5 focus:outline-none focus:border-green-400">
                                <button type="submit" name="add_rate"
                                        class="w-full bg-green-600 hover:bg-green-700 text-white font-bold py-2 rounded-xl text-xs transition">
                                    <i class="fas fa-plus mr-1"></i>Add Plan
                                </button>
                            </form>
                        </div>
            ...
            <?php if (!empty($packages)): ?>
                <div class="space-y-4">
                    <?php foreach ($packages as $pkg):
                        $slip_url  = !empty($pkg['Slip_payment'])
                            ? '/Badminton_court_Booking/assets/images/slips/' . basename($pkg['Slip_payment']) : '';
                        ...
                        <div class="pkg-card bg-white rounded-2xl shadow-sm border-l-4 <?= $status_cfg['border'] ?> p-5">
                            <div class="flex flex-col md:flex-row justify-between gap-4">
                                ...
                                <div class="flex items-start gap-4 flex-1 min-w-0">
                                    ...
                                    <div class="min-w-0">
                                        ...
                                        <p class="text-sm text-gray-500 mb-0.5">
                                            ...
                                            <?= !empty($pkg['VN_Name']) ? htmlspecialchars($pkg['VN_Name']) : '<span class="text-gray-400 italic">No venue yet</span>' ?>
                                        </p>
                                        <p class="text-sm text-gray-500 mb-0.5"><i class="fas fa-envelope mr-1 text-gray-400"></i><?= htmlspecialchars($pkg['owner_email']) ?></p>
                                        <p class="text-sm text-gray-500"><?php if ($pkg['owner_phone']): ?><i class="fas fa-phone mr-1 text-gray-400"></i><?= htmlspecialchars($pkg['owner_phone']) ?><?php endif; ?></p>
                                        ...
                                        <div class="mt-2 flex flex-wrap gap-2">
                                            <span class="inline-flex items-center gap-1 bg-gray-50 border border-gray-200 text-gray-600 text-xs font-semibold px-2.5 py-1 rounded-full">
                                                ...
                                                <?= htmlspecialchars($pkg['Package_duration']) ?>
                                            </span>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div class="flex flex-col items-end gap-1 text-right flex-shrink-0">
                                    ...
                                    <p class="text-xs text-gray-400">Submitted: <?= date('M d, Y g:i A', strtotime($pkg['Package_date'])) ?></p>
                                        ...
                                        <?php if ($is_queued): ?>
                                            <div class="mt-1 bg-purple-50 border border-purple-200 rounded-xl px-3 py-1.5 text-right">
                                                ...
                                                <p class="text-xs text-purple-500">
                                                    <?= date('M d, Y', strtotime($pkg['Start_time'])) ?> → <?= date('M d, Y', strtotime($pkg['End_time'])) ?>
                                                </p>
                                            </div>
                                        <?php else: ?>
                                            <p class="text-xs <?= $days_left <= 14 ? 'text-red-500 font-bold' : 'text-gray-400' ?>">
                                                <?= $days_left > 0 ? $days_left . ' days left' : 'Expired' ?> · Until <?= date('M d, Y', strtotime($pkg['End_time'])) ?>
                                            </p>
                                            <?php
                                                $total_days = max(1, (strtotime($pkg['End_time']) - strtotime($pkg['Start_time'])) / 86400);
                            ...
                            <?php if ($pkg['Status_Package'] === 'Rejected' && !empty($pkg['reject_reason'])): ?>
                                <div class="mt-3 bg-red-50 border border-red-200 rounded-xl px-4 py-3 flex items-start gap-2">
                                    ...
                                    <div>
                                        <p class="text-xs font-bold text-red-600 mb-0.5">Reason sent to owner:</p>
                            ...
                            <div class="flex flex-wrap gap-2 mt-4 pt-4 border-t border-gray-100">
                                ...
                                <?php if ($pkg['Status_Package'] === 'Rejected'): ?>
                                    <form method="POST" class="inline"
                                          onsubmit="return confirm('Permanently delete this rejected package record?')">
                                        <input type="hidden" name="pkg_id" value="<?= $pkg['Package_ID'] ?>">
            ...
            <?php else: ?>
                <div class="bg-white rounded-2xl shadow-sm p-12 text-center">
                    ...
                    <h3 class="text-xl font-bold text-gray-600 mb-2">No <?= ucfirst($filter) ?> Packages</h3>
                    <p class="text-gray-400 text-sm">
                        <?= $filter === 'pending' ? 'All package payments have been reviewed.' : 'No packages in this category.' ?>
                    </p>
                </div>
            <?php endif; ?>
...
<div class="slip-modal" id="slipModal">
    <div class="bg-white rounded-2xl shadow-2xl p-6 max-w-lg w-full mx-4 relative">
        ...
        <h3 class="font-bold text-gray-800 text-lg mb-4">
            <i class="fas fa-receipt text-blue-500 mr-2"></i>Payment Slip — Package #<span id="modalPkgId"></span>
        </h3>
...
<div class="reject-modal" id="rejectModal">
    <div class="bg-white rounded-2xl shadow-2xl p-6 max-w-md w-full mx-4 relative">
        ...
        <h3 class="font-bold text-gray-800 text-lg mb-1">
            <i class="fas fa-times-circle text-red-500 mr-2"></i>Reject Package
        </h3>
...
function toggleBadge(targetId, otherId, type, btn) {
...
function viewSlip(url, pkgId, status) {
    ...
    if (status === 'Pending') {
        actions.innerHTML = `
            ...
            <button onclick="closeSlip(); openReject(${pkgId})"
