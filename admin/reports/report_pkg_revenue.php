...
// Package revenue for selected period
try {
    $stmt = $pdo->prepare("
        SELECT COALESCE(SUM(pr.Price),0)
        FROM package bp
        INNER JOIN package_rate pr ON bp.Package_rate_ID=pr.Package_rate_ID
        WHERE bp.Status_Package IN ('Active','Expired') AND DATE(bp.Package_date)>=?
    ");
    $stmt->execute([$date_from]);
    $pkg_revenue = (float)$stmt->fetchColumn();
} catch (PDOException $e) { $pkg_revenue = 0; }

// Packages list for selected period
try {
    $stmt = $pdo->prepare("
        SELECT bp.Package_ID, bp.Package_date, bp.Status_Package,
               pr.Package_duration, pr.Price,
        ...
        WHERE DATE(bp.Package_date) >= ?
        ORDER BY bp.Package_date ASC LIMIT 100
    ");
    $stmt->execute([$date_from]);
    $all_packages = $stmt->fetchAll(PDO::FETCH_ASSOC);
} catch (PDOException $e) { $all_packages = []; }

$pkg_active  = count(array_filter($all_packages, fn($p) => $p['Status_Package'] === 'Active'));
$pkg_expired = count(array_filter($all_packages, fn($p) => $p['Status_Package'] === 'Expired'));

// Monthly package revenue
try {
    $stmt = $pdo->query("
        SELECT DATE_FORMAT(bp.Package_date,'%Y-%m') AS month,
               DATE_FORMAT(bp.Package_date,'%b %Y') AS month_label,
               SUM(pr.Price) AS revenue
        FROM package bp
        INNER JOIN package_rate pr ON bp.Package_rate_ID=pr.Package_rate_ID
        WHERE bp.Status_Package IN ('Active','Expired')
        GROUP BY DATE_FORMAT(bp.Package_date,'%Y-%m')
        ORDER BY month ASC LIMIT 12
    ");
    $pkg_monthly_rows = $stmt->fetchAll(PDO::FETCH_ASSOC);
} catch (PDOException $e) { $pkg_monthly_rows = []; }
...
<head>
    ...
    <title>ລາຍງານລາຍຮັບແພັກເກດ - Admin</title>
        ...
        <main class="flex-1 p-6">
            ...
            <div class="print-header mb-6 pb-4 border-b-2 border-gray-800">
                <div class="flex items-center justify-between">
                    <div>
                        <h1 class="text-2xl font-bold">ລາຍງານລາຍຮັບແພັກເກດ</h1>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
                <div class="stat-card bg-gradient-to-br from-purple-500 to-purple-600 rounded-2xl p-6 text-white">
                    ...
                    <p class="text-purple-100 text-sm mt-1">ລາຍຮັບແພັກເກດລວມ</p>
                    <p class="text-purple-200 text-xs"><?= $period_label ?></p>
                </div>
                <div class="stat-card bg-white rounded-2xl shadow-sm p-6 border border-gray-100">
                    ...
                    <p class="text-2xl font-extrabold text-gray-800"><?= $pkg_active ?></p>
                    <p class="text-xs font-semibold text-gray-600 mt-0.5">ແພັກເກດ Active</p>
                    <p class="text-xs text-gray-400">ໃຊ້ງານຢູ່</p>
                </div>
                <div class="stat-card bg-white rounded-2xl shadow-sm p-6 border border-gray-100">
                    <div class="bg-gray-100 w-10 h-10 rounded-xl flex items-center justify-center mb-3">
                        <i class="fas fa-history text-gray-500"></i>
                    </div>
                    <p class="text-2xl font-extrabold text-gray-800"><?= $pkg_expired ?></p>
                    <p class="text-xs font-semibold text-gray-600 mt-0.5">ແພັກເກດໝົດອາຍຸ</p>
                    <p class="text-xs text-gray-400">Expired</p>
                </div>
            </div>

            <!-- Monthly Chart -->
            <?php if (!empty($pkg_monthly_rows)): ?>
            <div class="bg-white rounded-2xl shadow-sm p-6 mb-6">
                <h2 class="font-bold text-gray-800 mb-4">
                    <i class="fas fa-chart-bar text-purple-500 mr-2"></i>ລາຍຮັບແພັກເກດລາຍເດືອນ
                </h2>
                <canvas id="pkgRevenueChart" height="90"></canvas>
            </div>
            <?php endif; ?>
            ...
            <div class="bg-white rounded-2xl shadow-sm p-6">
                <div class="flex items-center justify-between mb-4">
                    <h2 class="font-bold text-gray-800"><i class="fas fa-user-tie text-purple-400 mr-2"></i>ລາຍຮັບຕໍ່ເຈົ້າຂອງເດີ່ນ</h2>
                    <span class="text-sm text-gray-400"><?= count($all_packages) ?> ລາຍການ</span>
                </div>
                    ...
                    <table class="w-full text-sm">
                        ...
                        <tbody class="divide-y divide-gray-50">
                            <?php foreach ($all_packages as $pkg):
                                $pc = match($pkg['Status_Package']) {
                                    'Active'  => ['bg-green-100', 'text-green-700', 'ໃຊ້ງານ'],
                                    'Expired' => ['bg-gray-100',  'text-gray-500',  'ໝົດອາຍຸ'],
                                    default   => ['bg-yellow-100','text-yellow-700', $pkg['Status_Package']],
                                };
                            ?>
                                <tr class="hover:bg-gray-50 transition">
                                    <td class="py-3 px-3 text-gray-400 text-xs"><?= $pkg['Package_ID'] ?></td>
                                    <td class="py-3 px-3">
                                        <p class="font-semibold text-gray-800"><?= htmlspecialchars($pkg['owner_name']) ?></p>
                                        <p class="text-xs text-gray-400"><?= htmlspecialchars($pkg['owner_phone'] ?? '') ?></p>
                                    </td>
                                    <td class="py-3 px-3 text-gray-600 text-xs"><?= htmlspecialchars($pkg['VN_Name']) ?></td>
                                    <td class="py-3 px-3 text-center text-gray-500 text-xs"><?= htmlspecialchars($pkg['Package_duration']) ?></td>
                                    <td class="py-3 px-3 text-center text-gray-500 text-xs"><?= date('d/m/Y', strtotime($pkg['Package_date'])) ?></td>
                                    <td class="py-3 px-3 text-right font-extrabold text-purple-600">₭<?= number_format($pkg['Price']) ?></td>
                                    <td class="py-3 px-3 text-center">
                                        <span class="<?= $pc[0] ?> <?= $pc[1] ?> text-xs font-bold px-2 py-1 rounded-full"><?= $pc[2] ?></span>
                                    </td>
                                </tr>
                            <?php endforeach; ?>
                        </tbody>
                        <tfoot>
                            <tr class="border-t-2 border-gray-200 bg-gray-50">
                                <td colspan="5" class="py-3 px-3 font-bold text-gray-700">ລວມທັງໝົດ (Active + Expired)</td>
                                <td class="py-3 px-3 text-right font-extrabold text-purple-600">₭<?= number_format(array_sum(array_column($all_packages, 'Price'))) ?></td>
                                <td></td>
                            </tr>
                        </tfoot>
                    </table>
                ...
                <?php else: ?>
                    <div class="text-center py-12">
                        <i class="fas fa-box text-5xl text-gray-200 mb-3 block"></i>
                        <p class="text-gray-400">ບໍ່ມີແພັກເກດໃນໄລຍະນີ້</p>
                    </div>
                <?php endif; ?>
            </div>
...
<?php if (!empty($pkg_monthly_rows)): ?>
<script>
const pkgLabels  = <?= json_encode(array_column($pkg_monthly_rows, 'month_label')) ?>;
const pkgRevenue = <?= json_encode(array_column($pkg_monthly_rows, 'revenue')) ?>;

new Chart(document.getElementById('pkgRevenueChart').getContext('2d'), {
    type: 'bar',
    data: {
        labels: pkgLabels,
        datasets: [{
