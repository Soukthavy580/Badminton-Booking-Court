...
try {
    $stmt = $pdo->prepare("
        SELECT v.VN_ID, v.VN_Name, v.VN_Address, v.Price_per_hour, v.VN_Status,
        ...
        <main class="flex-1 p-6">
            ...
            <div class="print-header mb-6 pb-4 border-b-2 border-gray-800">
                <div class="flex items-center justify-between">
                    <div>
                        <h1 class="text-2xl font-bold">ລາຍງານຂໍ້ມູນເດີ່ນທັງໝົດ</h1>
            ...
            <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
                <?php foreach ([
                    ['label'=>'ສະຖານທີ່ໃຊ້ງານ','value'=>$total_venues,   'icon'=>'fa-store',        'color'=>'blue',  'sub'=>'Active'],
                    ['label'=>'ເດີ່ນທັງໝົດ',    'value'=>$total_courts,   'icon'=>'fa-table-tennis', 'color'=>'green', 'sub'=>'Active'],
                    ['label'=>'ເຈົ້າຂອງເດີ່ນ',  'value'=>$total_owners,   'icon'=>'fa-user-tie',     'color'=>'purple','sub'=>'ລົງທະບຽນ'],
                    ['label'=>'ລູກຄ້າທັງໝົດ',   'value'=>$total_customers,'icon'=>'fa-users',        'color'=>'yellow','sub'=>'ລົງທະບຽນ'],
                ] as $sc): ?>
                    ...
                    <table class="w-full text-sm">
                        <thead>
                            <tr class="border-b-2 border-gray-100 bg-gray-50">
                                <th class="text-left py-3 px-4 text-xs font-bold text-gray-500 uppercase">ສະຖານທີ່</th>
                                <th class="text-left py-3 px-4 text-xs font-bold text-gray-500 uppercase">ເຈົ້າຂອງ</th>
                                <th class="text-center py-3 px-4 text-xs font-bold text-gray-500 uppercase">ຈຳນວນຄອດ</th>
                        ...
                        <tbody class="divide-y divide-gray-50">
                            <?php foreach ($venue_stats as $v): ?>
                                <tr class="hover:bg-gray-50 transition">
                                    <td class="py-3 px-4 font-semibold text-gray-800"><?= htmlspecialchars($v['VN_Name']) ?></td>
                                    <td class="py-3 px-4 text-xs text-gray-500"><i class="fas fa-user-tie mr-1"></i><?= htmlspecialchars($v['owner_name']) ?></td>
                                    <td class="py-3 px-4 text-center"><span class="bg-blue-100 text-blue-700 text-xs font-bold px-2 py-1 rounded-full"><?= (int)$v['total_courts'] ?> ຄອດ</span></td>
                                    <td class="py-3 px-4 text-center text-gray-600"><?= number_format($v['total_bookings']) ?></td>
                                    <td class="py-3 px-4 text-center"><span class="bg-green-100 text-green-700 px-2 py-0.5 rounded-full text-xs font-bold"><?= number_format($v['confirmed']) ?></span></td>
                                    <td class="py-3 px-4 text-center"><span class="bg-red-100 text-red-600 px-2 py-0.5 rounded-full text-xs font-bold"><?= number_format($v['cancelled']) ?></span></td>
                                    <td class="py-3 px-4 text-right font-bold text-green-600"><?= $v['booking_revenue'] > 0 ? '₭'.number_format($v['booking_revenue']) : '—' ?></td>
                                </tr>
                            <?php endforeach; ?>
                            <?php if (empty($venue_stats)): ?>
                                <tr><td colspan="7" class="py-12 text-center text-gray-400">ຍັງບໍ່ມີຂໍ້ມູນ</td></tr>
                            <?php endif; ?>
                        </tbody>
                    </table>
                </div>
