...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['save_rate'])) {
    ...
    if ($rate_id && $duration && $price > 0) {
        ...
        } catch (PDOException $e) { $error = 'ລົ້ມເຫລວ: ' . $e->getMessage(); }
    } else { $error = 'ກະລຸນາໃສ່ໄລຍະ ແລະ ລາຄາ.'; }
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['delete_rate'])) {
    ...
    if ($rate_id) {
        try {
            // FIX: correct table = advertisement
        ...
        } catch (PDOException $e) { $error = 'ລົ້ມເຫລວ.'; }
    }
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && !isset($_POST['save_rate']) && !isset($_POST['add_rate']) && !isset($_POST['delete_rate']) && !isset($_POST['delete_rejected'])) {
    ...
    if ($ad_id && in_array($action, ['approve','reject'])) {
        try {
            ...
            ");
            $stmt->execute([$ad_id]);
            $ad = $stmt->fetch(); // FIX: was $pkg then used $ad
...
function get_ads($pdo, $filter) {
    // FIX: correct table = advertisement throughout
    $where = match($filter) {
...
try { $ads = get_ads($pdo, $filter); } catch (PDOException $e) { $ads = []; $error = 'Query error: '.$e->getMessage(); }
$counts = ['pending'=>count_ads($pdo,'pending'),'active'=>count_ads($pdo,'active'),'expired'=>count_ads($pdo,'expired'),'rejected'=>count_ads($pdo,'rejected')];
try { $ad_rates = $pdo->query("SELECT * FROM advertisement_rate ORDER BY Price ASC")->fetchAll(); } catch (PDOException $e) { $ad_rates = []; }
// FIX: correct table = advertisement
try { $revenue = $pdo->query("SELECT COALESCE(SUM(r.Price),0) FROM advertisement ad INNER JOIN advertisement_rate r ON ad.AD_Rate_ID=r.AD_Rate_ID WHERE ad.Status_AD IN ('Approved','Active')")->fetchColumn(); } catch (PDOException $e) { $revenue = 0; }
...
<head>
    ...
    <style>
        .pkg-card{transition:all .3s ease}.pkg-card:hover{transform:translateY(-2px);box-shadow:0 8px 20px rgba(0,0,0,.08)}
                    ...
                    <div class="p-6">
                        ...
                        <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4 mb-2">
                            <?php foreach ($ad_rates as $rate):
                                ...
                                $mo_price = preg_match('/(\d+)/', $rate['Duration'], $m) ? round($rate['Price'] / max(1, intval($m[1]))) : null;
                            ?>
                            ...
                            <div class="bg-white rounded-2xl border-2 border-dashed border-blue-200 p-4 flex flex-col items-center justify-center text-center gap-2 min-h-[260px]">
                                ...
                                <form method="POST" class="w-full space-y-1.5">
                                    ...
                                    <input type="text" name="rate_duration" placeholder="ໄລຍະ (ຕົວຢ່າງ: 3 Months)" class="w-full text-center text-xs border border-gray-200 rounded-lg px-2 py-1.5 focus:outline-none focus:border-blue-400">
            ...
            <?php if (!empty($ads)): ?>
                <div class="space-y-4">
                    <?php foreach ($ads as $ad):
                        ...
                        $is_queued = !empty($ad['Start_time']) && $ad['Start_time'] > $now_str && $ad['Status_AD'] === 'Active';
                            ...
                            <div class="flex flex-col md:flex-row justify-between gap-4">
                                <div class="flex items-start gap-4 flex-1 min-w-0">
                                    ...
                                    <div class="min-w-0">
                                        ...
                                        <?php if ($ad['owner_phone']): ?><p class="text-sm text-gray-500"><i class="fas fa-phone mr-1 text-gray-400"></i><?= htmlspecialchars($ad['owner_phone']) ?></p><?php endif; ?>
                                        <div class="mt-2"><span class="inline-flex items-center gap-1 bg-gray-50 border border-gray-200 text-gray-600 text-xs font-semibold px-2.5 py-1 rounded-full"><i class="fas fa-clock text-xs text-gray-400"></i><?= htmlspecialchars($ad['Duration']) ?></span></div>
                                    </div>
                                </div>
                                <div class="flex flex-col items-end gap-1 text-right flex-shrink-0">
                                    ...
                                    <?php if ($ad['Status_AD']==='Active' && $ad['End_time']): ?>
                                        <?php if ($is_queued): ?>
                                            <div class="mt-1 bg-purple-50 border border-purple-200 rounded-xl px-3 py-1.5 text-right">
                                                ...
                                                <p class="text-xs text-purple-500"><?= date('d/m/Y', strtotime($ad['Start_time'])) ?> → <?= date('d/m/Y', strtotime($ad['End_time'])) ?></p>
                                            </div>
                                        <?php else: ?>
                                            ...
                                            <?php $total_days = max(1,(strtotime($ad['End_time'])-strtotime($ad['Start_time']))/86400); $pct = min(100,max(0,($days_left/$total_days)*100)); ?>
                                            <div class="w-32 bg-gray-100 rounded-full h-1.5 mt-1"><div class="bg-green-500 h-1.5 rounded-full" style="width:<?= $pct ?>%"></div></div>
                                        <?php endif; ?>
                                    <?php endif; ?>
                                </div>
                            </div>
...
<script>
function toggleRates(){const p=document.getElementById('ratesPanel'),c=document.getElementById('ratesChevron');p.classList.toggle('hidden');c.style.transform=p.classList.contains('hidden')?'':'rotate(180deg)';}
function toggleBadge(tId,oId,type,btn){const t=document.getElementById(tId),o=document.getElementById(oId),on=t.value==='1';t.value=on?'0':'1';if(!on&&o)o.value='0';if(type==='popular'){btn.className=btn.className.replace(/(bg-blue-50 text-blue-600 border-blue-200|bg-gray-50 text-gray-400 border-gray-200)/g,'').trim();btn.className+=on?' bg-gray-50 text-gray-400 border-gray-200':' bg-blue-50 text-blue-600 border-blue-200';}else{btn.className=btn.className.replace(/(bg-yellow-50 text-yellow-600 border-yellow-300|bg-gray-50 text-gray-400 border-gray-200)/g,'').trim();btn.className+=on?' bg-gray-50 text-gray-400 border-gray-200':' bg-yellow-50 text-yellow-600 border-yellow-300';}}
function viewSlip(url,adId,status){document.getElementById('slipModal').classList.add('open');document.getElementById('modalSlipImg').src=url;document.getElementById('modalAdId').textContent=adId;const a=document.getElementById('modalActions');if(status==='Pending'){a.innerHTML=`<form method="POST" class="flex-1"><input type="hidden" name="ad_id" value="${adId}"><input type="hidden" name="action" value="approve"><button type="submit" class="w-full bg-green-600 hover:bg-green-700 text-white font-bold py-3 rounded-xl transition"><i class="fas fa-check mr-2"></i>ອະນຸມັດ</button></form><button onclick="closeSlip();openReject(${adId})" class="flex-1 bg-red-50 hover:bg-red-100 text-red-600 font-bold py-3 rounded-xl border border-red-200 transition"><i class="fas fa-times mr-2"></i>ປະຕິເສດ</button>`;}else a.innerHTML='';}
...
function openReject(id){document.getElementById('rejectAdId').value=id;document.querySelector('#rejectModal textarea').value='';document.getElementById('rejectModal').classList.add('open');}
